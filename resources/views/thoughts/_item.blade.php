@php
    ...
    $shortSlug = $shortSlug ?? false;

    $publishedAt = $content->createdAtInCreatedTimezone();
    $linkSlug = $shortSlug ? substr((string) $content->slug, 0, 12) : $content->slug;
@endphp
...
<article class="thoughts-item" data-thought-item data-thought-slug="{{ $content->slug }}"
    ...
    data-thought-view-fragment-url="{{ route('thoughts.fragments.view', ['slug' => $content->slug, 'wrapContent' => $wrapContent ? '1' : '0', 'shortSlug' => $shortSlug ? '1' : '0']) }}">
    <header class="thoughts-item__header">
        <a class="thoughts-item__time" href="{{ route('thoughts.show', ['slug' => $linkSlug]) }}">
    ...
    <div data-thought-view>
        @include('thoughts._view', ['content' => $content, 'wrapContent' => $wrapContent])
    </div>
